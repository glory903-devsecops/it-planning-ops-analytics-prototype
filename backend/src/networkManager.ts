...
export interface NetworkEvent {
  timestamp: string;
  service: string;
  status: '건강(정상)' | '지연(Latency)' | '장애(Timeout/502)';
  latencyMs: number;
  message: string;
}
...
export class NetworkManager {
  ...
  private generateRandomEvent(timestamp?: number): NetworkEvent {
    ...
    let status: '건강(정상)' | '지연(Latency)' | '장애(Timeout/502)' = '건강(정상)';
